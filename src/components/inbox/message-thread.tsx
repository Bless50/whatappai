...
export function MessageThread({
  ...
  const { user } = useAuth();
  const [isLinkedPhone, setIsLinkedPhone] = useState(false);
  ...
  useEffect(() => {
    ...
    const checkConfig = async () => {
      ...
      setIsLinkedPhone(data?.phone_number_id === "linked-phone");
    };
    checkConfig();
  }, [user]);
  ...
  const sessionInfo = useMemo(() => {
    if (isLinkedPhone) return { expired: false, remaining: "" };
